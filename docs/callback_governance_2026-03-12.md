...
| `admin_` | `admin_*`, `admin_nuevo_pedido(_{admin_id})?`, `admin_mi_saldo(_{admin_id})?`, `admin_movimientos(_(soc_)?(hoy|semana|mes|todo)(_{admin_id})?)?`, `admin_mis_clientes(_{admin_id})?`, `admin_mis_dirs(_{admin_id})?`, `admin_mis_plantillas(_{admin_id})?`, `admin_sociedad_retiro(_{admin_id})?`, `admin_pinissue_*`, `admin_ruta_pinissue_*`, `admin_support_(open|list_{offset}|view_{support_id}_{offset})` | `Backend/main.py`, `Backend/order_delivery.py`, `Backend/profile_changes.py`, `Backend/handlers/admin_panel.py`, `Backend/handlers/order.py`, `Backend/handlers/recharges.py`, `Backend/handlers/customer_agenda.py`, `Backend/handlers/location_agenda.py` | `admin_menu_callback`, `admin_nuevo_pedido_start`, `admin_mi_saldo_callback`, `admin_movimientos_callback`, `admin_clientes_cmd`, `admin_dirs_cmd`, `admin_mis_plantillas_callback`, `sociedad_retiro_iniciar_callback`, `order_courier_callback`, `handle_route_callback` | Documentado, pero muy amplio | Mantener tal como esta |
...
| `admin_pedido_` | `admin_pedido_(pickup_{id}|nueva_dir|geo_(si|no)|geo_pickup_(si|no)|sin_instruc|inc_(1000|1500|2000|3000|otro)|confirmar|cancelar|team_toggle|guardar_plantilla|edit_pickup|edit_cust|edit_tarifa)` | `Backend/main.py`, `Backend/handlers/order.py` | flujo de pedido especial del admin | Documentado | Mantener tal como esta |
| `agenda_` | `agenda_(pickups|clientes|cerrar|volver|pickups_nueva|pickup_(lista|ver_{id}|base_{id}|fix_{id}|del_{id}|del_confirm_{id}|del_cancel))` | `Backend/main.py` | `agenda_menu_callback`, `agenda_pickups_*` | Documentado | Mantener tal como esta |
...
| `ally_locs_` | `ally_locs_(lista|add|ver_{id}|base_{id}|fix_{id}|del_{id}|del_confirm_{id}|del_cancel)` | `Backend/handlers/location_agenda.py` | `ally_locs_menu_callback` | No documentado | Documentar tal como esta |
...
| `pedido_` | `pedido_(cliente_*|sel_*|instr_*|tipo_*|base_(si|no|20000|50000|100000|200000|otro)|retry_quote|inc_*|confirmar|cancelar|guardar_...|edit_pickup|edit_instruc|edit_dir)` | `Backend/main.py`, `Backend/order_delivery.py`, `Backend/handlers/order.py` | flujo de pedido, edicion de incentivo y edicion desde preview (IMPLEMENTADO 2026-04-07) | Documentado | Mantener tal como esta |
...
| `ruta_` | `ruta_(aceptar|rechazar|ocupado|entregar|liberar|liberar_motivo|liberar_confirmar|liberar_abort|arrival_*|find_another|find_another_confirm|find_another_abort|wait_courier|cancelar_aliado|cancelar_aliado_confirm|cancelar_aliado_abort|pinissue|pickup_*|sel_*|inc_(1000|1500|2000|3000|otro)|confirmar|cancelar|guardar_clientes_...|edit_pickup)` | `Backend/main.py`, `Backend/order_delivery.py`, `Backend/handlers/route.py` | `handle_route_callback` y flujo de creacion de ruta | Documentado | Mantener tal como esta |
