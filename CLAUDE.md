...
- `db.py:_init_db_postgres` — `psycopg2.errors.UndefinedColumn: column "courier_arrived_at" of relation "routes" does not exist`: la columna se agregó en las migraciones SQLite (`_init_db_sqlite`) pero nunca se incluyó la llamada equivalente `_pg_add_col("routes", "courier_arrived_at", "TIMESTAMP")` en `_init_db_postgres`. Síntoma: `assign_route_to_courier` fallaba en PostgreSQL (Railway PROD) al intentar aceptar cualquier ruta, dejando al courier con el mensaje de error genérico. Fix: agregar `_pg_add_col("routes", "courier_arrived_at", "TIMESTAMP")` junto a las demás migraciones de columnas de `routes`. Se ejecuta automáticamente al arrancar el bot (idempotente).
